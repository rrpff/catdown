...
Catdown is a Markdown editor designed for modern front-end development. It's designed to need nothing more than a `require` and a stylesheet for simplicity.

Built on [CodeMirror](http://codemirror.net) and not dependent on jQuery.


### Installation

Install it with NPM and require it in your code. [Usage examples](#usage) below.

```bash
$ npm install --save catdown
```

Catdown requires styyling. Unfortunately NPM isn't great for stylesheets or other assets. You can import the default `catdown` theme like this:

```html
<link rel="stylesheet" href="/node_modules/catdown/css/catdown.min.css">
```
...
See the [styling](#styling) section for more information.
...
var editor = new Catdown({
    ...
    // Set the initial value as file contents
    value: fs.readFileSync("README.md").toString(),
...
* The `CodeMirror` instance that the Catdown instance is based on. Catdown has some shorthand functions like `#keys` and `#value` that delegate to this, but you can access it directly if you need more advanced functionality.

> Note: it's generally better to use `Catdown#keys` instead of `CM.setOption("extraKeys")`, as the former extends the current keymap instead of replacing it entirely.
...
Most of the Catdown plugins available at the time of writing do little more than define and export a function.
...
Catdown is just a wrapper around CodeMirror designed specifically for Markdown, so a **huge** thanks to [Marijn Haverbeke](https://github.com/marijnh) and all the [CodeMirror](http://github.com/codemirror/CodeMirror) contributors. CodeMirror is awesome!

### Contribution

Want to help?

* Review the code and file an issue or get in touch with me if you have any queries, suggestions or if you find any bugs or 'errors in judgement'.
